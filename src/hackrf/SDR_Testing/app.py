...
def results(job):
	...
	charts = {}
	###Build SSIDs Per Channel Pie Chart
	pie_chart = {}
	collect_counts = {}
	pie_chart['colors'] = []
	...
	for i in range(0, len(collect_counts)):
		pie_chart['colors'].append("rgb({0}, {1}, {2})".format(randrange(0, 255), randrange(0, 255), randrange(0, 255)))
	pie_chart['counts'] = collect_counts.values()
	charts['ssids_per_channel'] = pie_chart

	###Build Packets Per Channel Bar Chart
	bar_chart = {}
	collect_counts = {}
	bar_chart['colors'] = []
	for record in wifi_records:
	...
	for i in range(0, len(collect_counts)):
		bar_chart['colors'].append("rgb({0}, {1}, {2})".format(randrange(0, 255), randrange(0, 255), randrange(0, 255)))
	bar_chart['counts'] = collect_counts.values()
	charts['packets_per_channel'] = bar_chart
